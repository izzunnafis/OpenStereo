...
class Aggregation(nn.Module):
    ...
    def process_corr(self, features_left, features_right, stage_idx):
        ...
        N, C, H, W = features_left.size()

        position_encoding = PositionEncodingSine(d_model=C, max_shape=(H, W))
        x_tmp_left = position_encoding(features_left)
        x_tmp_right = position_encoding(features_right)

        feat_left = torch.reshape(
            torch.permute(x_tmp_left, (0, 2, 3, 1)), 
            (N, H*W, C)
        )
        feat_right = torch.reshape(
            torch.permute(x_tmp_right, (0, 2, 3, 1)),
            (N, H*W, C)
        )
