...
class DispAtt(nn.Module):
    ...
    """
    def __init__(self, d_model, nhead, attention, max_disp=64):
        super(DispAtt, self).__init__()
        self.d_model = d_model
        self.nhead = nhead
        self.transformer_layer = LocalFeatureTransformer(d_model, nhead, ["self_single"], attention)
    ...
    def forward(self, feat):
        ...
        self.position_encoding = PositionEncodingSine(self.d_model, max_shape=(H, W))

        x = self.position_encoding(feat)
        feat = torch.reshape(
            torch.permute(x, (0, 2, 3, 1)), (N, H * W, C)
        )

        feat, _ = self.transformer_layer(feat, feat)

        feat = self.norm(feat)

        feat = torch.permute(torch.reshape(feat, (N, H, W, C)), (0, 3, 1, 2))

        feat = F.softmax(feat/(C**0.5), dim=1)


        # Disparity regression via soft-argmax
        disp_values = (torch.arange(C, device=feat.device)/C*self.max_disp).view(1, C, 1, 1)
        disp_map = (feat * disp_values).sum(dim=1).view(N,1 ,H, W)
