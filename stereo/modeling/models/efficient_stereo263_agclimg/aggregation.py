...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[25,16,9], corr_split_mode = [1,1,1], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        # self.corr_disp = [self.group_wise_split_num[0]*self.search_num[0] + max_disp//4,
        #                   self.group_wise_split_num[1]*self.search_num[1] + max_disp//8,
        #                   self.group_wise_split_num[2]*self.search_num[2] + max_disp//16]
        self.disp_stage = [max_disp//4, max_disp//8, max_disp//16]
        ...
        down0 = [hourglass(self.attention_channels[0]) for i in range(2)]
        ...
        down1 = [hourglass(self.attention_channels[1]) for i in range(2)]
        ...
        conv2 = [MobileV2Residual(self.attention_channels[2], self.attention_channels[2], stride=1, expanse_ratio=4)
                    for i in range(3)]
        # conv2 = [hourglass(self.attention_channels[2]) for i in range(3)]
        self.conv2 = nn.Sequential(*conv2)

        self.agg0 = AggregationLayer(self.attention_channels[0])
        self.agg1 = AggregationLayer(self.attention_channels[1])
    ...
    def process_corr(self, features_left, features_right, left_img, right_img, stage_idx):
        ...
        # down_left = F.interpolate(left_img, scale_factor=1/self.downsample_scale[stage_idx], mode='bilinear', align_corners=False)
        # down_right = F.interpolate(right_img, scale_factor=1/self.downsample_scale[stage_idx], mode='bilinear', align_corners=False)
        # corr_res2 = correlation_volume(down_left, down_right, self.disp_stage[stage_idx])

        # corr_res = torch.cat([corr_res1, corr_res2], dim=1)
...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv0 = BasicConv(in_channels, in_channels, is_3d=False, kernel_size=3, stride=1, padding=1)

        self.conv1 = nn.Sequential(BasicConv(in_channels, in_channels*2, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=2, dilation=1),
                                   BasicConv(in_channels*2, in_channels*2, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=1, dilation=1))
                                    
        self.conv2 = nn.Sequential(BasicConv(in_channels*2, in_channels*4, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=2, dilation=1),
                                   BasicConv(in_channels*4, in_channels*4, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=1, dilation=1))                             

        self.conv3 = nn.Sequential(BasicConv(in_channels*4, in_channels*8, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=2, dilation=1),
                                   BasicConv(in_channels*8, in_channels*8, is_3d=False, relu=True, kernel_size=3,
                                             padding=1, stride=1, dilation=1)) 


        self.conv3_up = BasicConv(in_channels*8, in_channels*4, deconv=True, is_3d=False,
                                  relu=True, kernel_size=4, padding=1, stride=2, dilation=1)

        self.conv2_up = BasicConv(in_channels*4, in_channels*2, deconv=True, is_3d=False,
                                  relu=True, kernel_size=4, padding=1, stride=2, dilation=1)

        self.conv1_up = BasicConv(in_channels*2, in_channels, deconv=True, is_3d=False, IN=False,
                                  relu=False, kernel_size=4, padding=1, stride=2, dilation=1)
...
class AggregationLayer(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.left1 = nn.Sequential(
            ...
            nn.BatchNorm2d(dim),
            nn.Conv2d(
                dim, dim, kernel_size=1, stride=1,
    ...
    def forward(self, x_d, x_s):
        ...
        left = left1 * torch.sigmoid(right2)
        right = right1 * torch.sigmoid(left2)
