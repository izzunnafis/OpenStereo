...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[9,25,25], corr_split_mode = [1,0,0], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        self.attdisp1 = VisionFoundationLayerDisp(window_size=8, dim=1, num_heads=1, vision_layer_type="attention")
        ...
        self.attdisp2 = VisionFoundationLayerDisp(window_size=8, dim=1, num_heads=1, vision_layer_type="attention")
        ...
        self.attdisp3 = VisionFoundationLayerDisp(window_size=8, dim=1, num_heads=1, vision_layer_type="attention")
        ...
        self.attdisp1_0 = VisionFoundationLayer(window_size=8, dim=self.attention_channels[0], num_heads=8, vision_layer_type="attention")
        ...
        self.attdisp1_1 = VisionFoundationLayer(window_size=4, dim=self.attention_channels[1], num_heads=8, vision_layer_type="attention")
        ...
        self.attdisp2_0 = VisionFoundationLayer(window_size=8, dim=self.attention_channels[0], num_heads=8, vision_layer_type="attention")
    ...
    def forward(self, features_left, features_right):
        for i in range(features_left.__len__()):
            features_left[i] = self.conv_fn1[i](features_left[i])
            features_right[i] = self.conv_fn1[i](features_right[i])

        corr0 = self.process_corr(features_left[0], features_right[0], 0) #N, search_num*group_wise_split_num, H, W
        corr1 = self.process_corr(features_left[1], features_right[1], 1) #N, search_num*group_wise_split_num, H, W
        ...
        vol0_conv = self.conv0(vol0) #N, attention_channels[0], H, W
        vol0_att = self.attdisp1(vol0) #N, attention_channels[0], H, W
        vol0 = vol0_conv + vol0_att
        vol1 = self.conv1_init(corr1) #N, attention_channels[1], H, W
        vol1_conv = self.conv1(vol1) #N, attention_channels[1], H, W
        vol1_att = self.attdisp2(vol1) #N, attention_channels[1], H, W
        vol1 = vol1_conv + vol1_att
        vol2 = self.conv2_init(corr2) #N, attention_channels[2], H, W
        vol2_conv = self.conv2(vol2) #N, attention_channels[2], H, W
        vol2_att = self.attdisp3(vol2) #N, attention_channels[2], H, W
        vol2 = vol2_conv + vol2_att
        ...
        vol_att12 = F.relu(self.conv1_1(vol_att12) + self.attdisp1_1(vol_att12) + self.redir1(vol_att1), inplace=True) #N, attention_channels[1], H, W

        vol_att0 = self.attdisp1_0(vol_att0) + self.conv1_0(vol_att0) #N, attention_channels[0], H, W

        vol_att012 = self.fpn_layer1(vol_att12, vol_att0) #N, attention_channels[0], H, W
        vol_att012 = F.relu(self.attdisp2_0(vol_att012) + self.conv2_0(vol_att012) + self.redir0(vol_att0), inplace=True) #N, attention_channels[0], H, W
