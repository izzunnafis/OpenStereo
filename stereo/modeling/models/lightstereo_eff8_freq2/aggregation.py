...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[25,25,25], corr_split_mode = [1,1,1], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        self.conv_no0 = nn.Conv2d(self.attention_channels[0], self.attention_channels[0], 3, padding=1, stride=1, bias=False)
        self.conv_up11 = nn.ConvTranspose2d(self.attention_channels[1], self.attention_channels[1], 2, padding=0, output_padding=0, stride=2, bias=False)
        self.conv_up21 = nn.ConvTranspose2d(self.attention_channels[2], self.attention_channels[2], 2, padding=0, output_padding=0, stride=2, bias=False)
        self.conv_up22 = nn.ConvTranspose2d(self.attention_channels[2], self.attention_channels[2], 2, padding=0, output_padding=0, stride=2, bias=False)

        att_channel_sum = self.attention_channels[0] + self.attention_channels[1] + self.attention_channels[2]
        self.feat_cat_conv = MobileV2Residual(att_channel_sum, att_channel_sum, stride=1, expanse_ratio=4)
        self.feat_cat_conv2 = MobileV2Residual(att_channel_sum, self.attention_channels[0], stride=1, expanse_ratio=4)
    ...
    def forward(self, features_left, features_right, left_img, right_img):
        ...
        feat_l2_u1 = self.conv_up21(feat_l2) #N, attention_channels[2], H*2, W*2
        feat_l2_u2 = self.conv_up22(feat_l2_u1) #N, attention_channels[2], H*2, W*2
        feat_l1_u1 = self.conv_up11(feat_l1) #N, attention_channels[1], H*2, W*2
        feat_l0 = self.conv_no0(feat_l0) #N, attention_channels[0], H, W

        feat_cat = torch.cat([feat_l0, feat_l1_u1, feat_l2_u2], dim=1) #N, attention_channels[0]+attention_channels[1]+attention_channels[2], H, W
        feat_cat = self.feat_cat_conv(feat_cat) #N, attention_channels[0]+attention_channels[1]+attention_channels[2], H, W
        feat_cat = self.feat_cat_conv2(feat_cat) #N, attention_channels[0], H, W
        feat_l = F.sigmoid(feat_cat) #N, attention_channels[0]+attention_channels[1]+attention_channels[2], H, W
        feat_h = 1-F.sigmoid(feat_cat) #N, attention_channels[0]+attention_channels[1]+attention_channels[2], H, W

        disp0 = self.conv0_init(corr0) #N, attention_channels[0], H, W
        disp0 = self.conv0(disp0) #N, attention_channels[0], H, W

        disp0_l = disp0 * feat_l #N, attention_channels[0], H, W
        disp0_h = disp0 * feat_h #N, attention_channels[0], H, W

        disp0_l = self.conv_l(disp0_l) #N, attention_channels[0], H, W
        disp0_h = self.conv_h(disp0_h) #N, attention_channels[0], H, W

        disp_all = F.relu(disp0_l + disp0_h, inplace=True) #N, attention_channels[0], H, W
