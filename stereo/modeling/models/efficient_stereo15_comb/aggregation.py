...
from stereo.modeling.cost_volume.cost_volume import correlation_volume, build_concat_volume
...
from .correlation import AGCL, CorrAsAttention
...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[9,25,25], corr_split_mode = [1,0,0], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        self.corr_disp = [max_disp // 4, max_disp // 8, (max_disp // 16)]

        self.corr_as_att = nn.ModuleList()
        for i in range(self.input_channel.__len__()):
            self.corr_as_att.append(
                CorrAsAttention(self.input_channel[i], heads=4, head_dim=16, max_disp=self.corr_disp[i])
            )
        
        self.concat_conv2d = nn.Conv2d(self.input_channel[2], self.input_channel[2]//4, 3, stride=1, padding=1)
        self.concat_conv3d = nn.Sequential(
            nn.Conv3d(self.input_channel[2]//2, self.input_channel[2]//4, (1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)),
            nn.BatchNorm3d(self.input_channel[2]//4),
            nn.ReLU(),
        )

        self.conv_fn1 = nn.ModuleList()
        for i in range(self.input_channel.__len__()):
        ...
        self.conv2_init = MobileV2Residual(self.corr_disp[2]*self.input_channel[2]//4, self.attention_channels[2], stride=1, expanse_ratio=4)
    ...
    def process_corr(self, features_left, features_right, stage_idx):

        # Compute correlation and cost volume
        # corr_res = correlation_volume(features_left, features_right, self.corr_disp[stage_idx])
        # B, C, H, W = features_left.shape

        # corr_res = self.concat_module[stage_idx](corr_res) #N, input_channel, max_disp, H, W
        # corr_res = corr_res.reshape(B, -1, H, W) #N, input_channel*max_disp, H, W
        ...
        if stage_idx == 0:
            ...
            corr_res = self.corr_as_att[stage_idx](features_left, features_right) #N, search_num*group_wise_split_num, H, W
        elif stage_idx == 2:
            features_left = self.concat_conv2d(features_left)
            features_right = self.concat_conv2d(features_right)
            corr_res = build_concat_volume(features_left, features_right, self.corr_disp[stage_idx])        
            corr_res = self.concat_conv3d(corr_res) #N, input_channel*max_disp, H, W
            corr_res = corr_res.reshape(corr_res.shape[0], -1, corr_res.shape[3], corr_res.shape[4]) #N, input_channel*max_disp, H, W
        return corr_res

    def forward(self, features_left, features_right):
        features_left_ = [None] * features_left.__len__()
        features_right_ = [None] * features_right.__len__()
        for i in range(features_left.__len__()):
            features_left_[i] = self.conv_fn1[i](features_left[i])
            features_right_[i] = self.conv_fn1[i](features_right[i])

        corr0 = self.process_corr(features_left_[0], features_right_[0], 0) #N, search_num*group_wise_split_num, H, W
        corr1 = self.process_corr(features_left_[1], features_right_[1], 1) #N, search_num*group_wise_split_num, H, W
        corr2 = self.process_corr(features_left_[2], features_right_[2], 2) #N, search_num*group_wise_split_num, H, W
