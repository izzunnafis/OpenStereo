...
class Aggregation2(nn.Module):
    def __init__(self, in_channels, left_att, blocks, expanse_ratio, backbone_channels):
        ...
        conv0 = [MobileNextResidual(in_channels, in_channels, stride=1, expanse_ratio=self.expanse_ratio)
                 for i in range(1)]
        self.conv0 = nn.Sequential(*conv0)

        self.conv1_l = MobileNextResidual(in_channels, in_channels, stride=2, expanse_ratio=self.expanse_ratio)

        self.conv2_l = MobileNextResidual(in_channels, in_channels, stride=2, expanse_ratio=self.expanse_ratio)
        conv3 = [MobileNextResidual(in_channels, in_channels, stride=1, expanse_ratio=self.expanse_ratio)
                     for i in range(4)]
        ...
        conv1_h = [MobileNextResidual(in_channels, in_channels, stride=1, expanse_ratio=self.expanse_ratio)
                 for i in range(4)]
