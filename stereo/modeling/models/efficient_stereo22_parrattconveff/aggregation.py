...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[9,25,25], corr_split_mode = [1,0,0], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        conv0 = [MobileV2Residual(self.attention_channels[0], self.attention_channels[0], stride=1, expanse_ratio=2)
                 for i in range(1)]
        ...
        conv1 = [MobileV2Residual(self.attention_channels[1], self.attention_channels[1], stride=1, expanse_ratio=2)
                    for i in range(1)]
        ...
        conv2 = [MobileV2Residual(self.attention_channels[2], self.attention_channels[2], stride=1, expanse_ratio=2)
                    for i in range(1)]
        ...
        conv1_0 = [MobileV2Residual(self.attention_channels[0], self.attention_channels[0], stride=1, expanse_ratio=2)
                    for i in range(2)]
        self.conv1_0 = nn.Sequential(*conv1_0)

        conv1_1 = [MobileV2Residual(self.attention_channels[1], self.attention_channels[1], stride=1, expanse_ratio=2)
                       for i in range(2)]
        self.conv1_1 = nn.Sequential(*conv1_1)

        conv2_0 = [MobileV2Residual(self.attention_channels[0], self.attention_channels[0], stride=1, expanse_ratio=2)
                    for i in range(2)]
