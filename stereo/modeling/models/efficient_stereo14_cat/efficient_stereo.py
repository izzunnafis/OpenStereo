...
from stereo.modeling.models.efficient_stereo14_cat.backbone import Backbone

# from .backbone import Backbone
from .aggregation import Aggregation, FPNLayer


class EfficientStereo14(nn.Module):
    ...
    def forward(self, data):
        ...
        encoded_vol, disp_att4, disp_att8, disp_att16 = self.cost_agg(features_left, features_right) # [bz, self.attention_channels[0], H/4, W/4]
        ...
        if self.training:
            ...
            disp_4 *= 4
            disp_att4 = disp_att4.unsqueeze(1)
            disp_att8 = disp_att8.unsqueeze(1)
            disp_att16 = disp_att16.unsqueeze(1)
            disp_att4 = F.interpolate(disp_att4, image1.shape[2:], mode='bilinear', align_corners=False)
            disp_att8 = F.interpolate(disp_att8, image1.shape[2:], mode='bilinear', align_corners=False)
            disp_att16 = F.interpolate(disp_att16, image1.shape[2:], mode='bilinear', align_corners=False)
            result['disp_4'] = disp_4
            result['disp_att4'] = disp_att4
            result['disp_att8'] = disp_att8
            result['disp_att16'] = disp_att16
    ...
    def get_loss(self, model_pred, input_data):
        ...
        disp_att4 = model_pred['disp_att4']
        disp_att4 = torch.clamp(disp_att4, min=1e-6, max=self.max_disp)
        if torch.isnan(disp_att4).any() or torch.isinf(disp_att4).any():
            print('disp_att4 has nan or inf')
            disp_att4 = torch.nan_to_num(disp_att4, nan=1e-6, posinf=self.max_disp, neginf=1e-6)
        loss += 0.3 * F.smooth_l1_loss(disp_att4[mask], disp_gt[mask], reduction='mean')

        disp_att8 = model_pred['disp_att8']
        disp_att8 = torch.clamp(disp_att8, min=1e-6, max=self.max_disp)
        if torch.isnan(disp_att8).any() or torch.isinf(disp_att8).any():
            print('disp_att8 has nan or inf')
            disp_att8 = torch.nan_to_num(disp_att8, nan=1e-6, posinf=self.max_disp, neginf=1e-6)
        loss += 0.3 * F.smooth_l1_loss(disp_att8[mask], disp_gt[mask], reduction='mean')

        disp_att16 = model_pred['disp_att16']
        disp_att16 = torch.clamp(disp_att16, min=1e-6, max=self.max_disp)
        if torch.isnan(disp_att16).any() or torch.isinf(disp_att16).any():
            print('disp_att16 has nan or inf')
            disp_att16 = torch.nan_to_num(disp_att16, nan=1e-6, posinf=self.max_disp, neginf=1e-6)
        loss += 0.3 * F.smooth_l1_loss(disp_att16[mask], disp_gt[mask], reduction='mean')
