...
from stereo.modeling.models.lightstereo.backbone import Backbone
...
class EfficientStereo(nn.Module):
    def __init__(self, cfgs):
        ...
        # self.final_reg = nn.Linear(18, 1)
        self.final_req = nn.Conv2d(18, 1, 3, stride=1, padding=1)
    ...
    def forward(self, data):
        ...
        N, H, W = image1.size()[0], image1.size()[2], image1.size()[3]
        for i in range(len(attn_weight)):
            # disp_val = self.final_reg(attn_weight[i])
            ...
            #                     (0,3,1,2),
            #                 )
            disp_val = self.final_req(attn_weight[i])
            out.append(disp_val)
