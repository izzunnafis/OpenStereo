...
def stride_partition(x, window_size):
    """
    Args:
        x: (B, C, H, W)
        window_size: window size
...
class AttentionMixerCross(nn.Module):
    ...
    def forward(self, left_feat, right_feat):
        B, N, C = left_feat.shape
        q = self.q(left_feat).reshape(B, N, 1, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        kv = self.kv(right_feat).reshape(B, N, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q = q.unbind(0)
        k, v = kv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
...
class VisionFoundationLayer(nn.Module):
    """
...
class VisionFoundationLayerCross(nn.Module):
    """
    VisionFoundation layer"
    """
    def __init__(self,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        if vision_layer_type == "attention":
            self.main_block = AttentionMixerCross(
                dim=dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
                qk_norm=qk_scale,
    ...
    def forward(self, left_feat, right_feat, corr_res):
        B, C, H, W = left_feat.shape
        left_feat = channel_partition(left_feat)
        right_feat = channel_partition(right_feat)

        left_feat_norm = self.norm1(left_feat)
        right_feat_norm = self.norm1(right_feat)
        x = self.main_block(left_feat_norm, right_feat_norm)

        #Reverse partition
        x = channel_reverse(x, B, H, W, C)
        
        stage_output  = x + corr_res
