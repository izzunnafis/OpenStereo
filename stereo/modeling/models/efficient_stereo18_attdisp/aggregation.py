...
from .mamba import VisionFoundationLayerDisp
...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[9,25,25], corr_split_mode = [1,0,0], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        conv1_0 = [VisionFoundationLayerDisp(window_size=8, dim=1, num_heads=1, vision_layer_type="attention")
                 for i in range(2)]
        self.conv1_0 = nn.Sequential(*conv1_0)

        conv1_1 = [VisionFoundationLayerDisp(window_size=4, dim=1, num_heads=1, vision_layer_type="attention")
                 for i in range(2)]
        self.conv1_1 = nn.Sequential(*conv1_1)

        conv2_0 = [VisionFoundationLayerDisp(window_size=8, dim=1, num_heads=1, vision_layer_type="attention")
                 for i in range(2)]
