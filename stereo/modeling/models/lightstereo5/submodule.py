...
class LearnableVarianceFilter(torch.nn.Module):
    def __init__(self, window_size, channel_size):
        ...
        self.kernel = torch.nn.Parameter(
            torch.ones((channel_size, 1, window_size, window_size))/window_size**2
        )
    ...
    def forward(self, image):
        ...
        return local_variance  # Apply sigmoid to normalize the output
...
def local_variance_filter(image, window_size):
    ...
    """
    B, C, H, W = image.shape
    kernel = torch.ones((C, 1, window_size, window_size), device=image.device) / (window_size ** 2)
    ...
    local_variance = local_variance# * 255
