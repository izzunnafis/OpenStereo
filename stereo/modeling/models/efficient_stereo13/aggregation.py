...
class Aggregation(nn.Module):
    def __init__(self, input_channel=[24, 32, 96], group_wise_split_num=[4,4,4], search_num=[9,25,25], corr_split_mode = [1,0,0], downsample_scale=[4, 8, 16], max_disp=192):
        ...
        self.concat_module = nn.ModuleList()
        for i in range(self.input_channel.__len__()):
            self.concat_module.append(
                nn.Sequential(
                    nn.Conv3d(self.input_channel[i]//2, self.input_channel[i]//4, 3, padding=1),
                    nn.BatchNorm3d(self.input_channel[i]//4),
                    nn.ReLU()
                )
            )
        ...
        for i in range(self.input_channel.__len__()):
            self.conv_fn1.append(nn.Conv2d(self.input_channel[i], self.input_channel[i]//4, 3, stride=1, padding=1))

        self.conv0_init = MobileV2Residual(self.corr_disp[0]*self.input_channel[0]//4, self.attention_channels[0], stride=1, expanse_ratio=4)
        ...
        self.conv1_init = MobileV2Residual(self.corr_disp[1]*self.input_channel[1]//4, self.attention_channels[1], stride=1, expanse_ratio=4)
